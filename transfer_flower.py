...
def get_or_create_bottleneck(sess,image_dir, image_lists, label_name, index, category, jpeg_data_tensor, bottleneck_tensor):
    # 获取一张图片对应的特征向量文件的路径。
    label_lists = image_lists[label_name]
    sub_dir = label_lists['dir']
    ...
    if not os.path.exists(bottleneck_path):
        ...
        # 获取原始的图片路径
        image_path = get_image_path(image_lists, image_dir, label_name, index, category)
        # 获取图片内容。
...
def get_random_cached_bottlenecks(sess, image_dir, n_classes, image_lists, how_many, category,
    ...
    for _ in range(how_many):
        ...
        bottleneck = get_or_create_bottleneck(sess, image_dir, image_lists, label_name, image_index, category,
                                              jpeg_data_tensor, bottleneck_tensor)
...
def get_test_bottlenecks(sess, image_dir, image_lists, n_classes, jpeg_data_tensor, bottleneck_tensor):
    bottlenecks = []
    ground_truths = []
    ...
    for label_index, label_name in enumerate(label_name_list):
        ...
        for index, unused_base_name in enumerate(image_lists[label_name][category]):
            ...
            bottleneck = get_or_create_bottleneck(sess, image_dir, image_lists, label_name, index, category,
                                                  jpeg_data_tensor, bottleneck_tensor)
            ground_truth = np.zeros(n_classes, dtype = np.float32)
            ground_truth[label_index] = 1.0
            bottlenecks.append(bottleneck)
...
def save_graph_to_file(sess, graph, graph_file_name):
  output_graph_def = graph_util.convert_variables_to_constants(
      sess, graph, ["final_training_ops/Softmax"])
...
def main(args):
    ...
    train_step = tf.train.GradientDescentOptimizer(args.learning_rate).minimize(cross_entropy_mean)
    # 计算正确率
    ...
    with tf.Session() as sess:
        if os.path.exists('model/checkpoint'):              #判断模型是否存在
            latest_ckpt = tf.train.latest_checkpoint('model/') #存在就从模型中恢复变量 
            if latest_ckpt:
                print(latest_ckpt) 
        ...
        for i in range(STEPS):
            ...
            train_bottlenecks, train_ground_truth = get_random_cached_bottlenecks(
                sess,INPUT_DATA, n_classes, image_lists, BATCH, 'training', jpeg_data_tensor, bottleneck_tensor)
...
#             print(train_bottlenecks.shape)
            ...
            if i%100 == 0 or i+1 == STEPS:
                validation_bottlenecks, validation_ground_truth = get_random_cached_bottlenecks(
                    sess,INPUT_DATA, n_classes, image_lists, BATCH, 'validation', jpeg_data_tensor, bottleneck_tensor)
        ...
        # 在最后的测试数据上测试正确率
        test_bottlenecks, test_ground_truth = get_test_bottlenecks(sess, INPUT_DATA,image_lists, n_classes,
                                                                       jpeg_data_tensor, bottleneck_tensor)
        ...
        # constants.
        save_graph_to_file(sess,sess.graph_def ,"tmp/frozen_graph.pb")
        with gfile.FastGFile("tmp/retrained_labels.txt", 'w') as f:
